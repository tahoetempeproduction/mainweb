...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwzj0w8IRDhbBKce-VfOgqrB7NZfH7SnBg5b39Qyuk9Z2nwHppueYa3q8g2rhLLC_bw6w/exec';
    ...
    nameInput.addEventListener('input', () => {
        nameInput.value.trim().length < 2 ? showError('name', 'Nama harus minimal 2 karakter') : hideError('name');
    });

    emailInput.addEventListener('input', () => {
        !validateEmail(emailInput.value) ? showError('email', 'Format email tidak valid') : hideError('email');
    });

    messageInput.addEventListener('input', () => {
        messageInput.value.trim().length < 10 ? showError('message', 'Pesan harus minimal 10 karakter') : hideError('message');
    });
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const errorMessageText = error.message.includes('HTTP') ? `Server error: ${error.message}` : 'Tidak dapat terhubung ke server. Periksa koneksi internet Anda.';
            document.getElementById('error-text').textContent = errorMessageText;
